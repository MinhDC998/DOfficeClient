import React, { useEffect } from 'react';
import { Card, Button, Table, ListGroup, ListGroupItem } from '@themesberg/react-bootstrap';


import { Link } from 'react-router-dom';
import { Routes } from '../../../routes';
import Preloader from "../../../components/Preloader";

import * as userApi from '../../../services/user';
import TableRow from "./TableRow";
import taskServices from '../../../services/task.services';

export default () => {
  const [tasks, setTasks] = React.useState();

  useEffect(() => {

    getListTask()
  }, [])

  const getListTask = async () => {
    const data = await taskServices.getAll();

    setTasks(data);
  }

  // console.log(tasks);



  return (
    <>
      <div className="" style={{ width: "25%" }}>
        <ListGroup horizontal>
          <ListGroup.Item action href={Routes.WorkManagement.path} >
            Work

          </ListGroup.Item>
          <ListGroup.Item action href={Routes.TaskManagement.path} active >
            Task
          </ListGroup.Item>
        </ListGroup>
      </div>

      <div className="d-xl-flex justify-content-between flex-wrap flex-md-nowrap align-items-center py-4" style={{ marginBottom: '25px' }}>

        <div className="d-block mb-4 mb-xl-0">
          <h4>Tasks</h4>
          {/* <p className="mb-0">
                    Dozens of reusable components built to provide buttons, alerts, popovers, and more.
                </p> */}

          <Button variant="secondary" className="m-1 mb-4">
            <Link to={Routes.AddWork.path}> Creat new work</Link>
          </Button>
        </div>
      </div>

      {tasks ? (
        <Card border="light" className="shadow-sm">
          <Card.Body className="p-0">
            <Table responsive className="table-centered rounded" style={{ whiteSpace: 'pre-wrap', wordWrap: 'break-word' }}>
              <thead className="thead-light ">
                <tr>
                  <th className="border-0" style={{ width: '15%' }}>Title</th>
                  <th className="border-0" style={{ width: '15%' }}>Work Id</th>
                  <th className="border-0" style={{ width: '15%' }}>Priority</th>
                  <th className="border-0" style={{ width: '15%' }}>Status</th>
                  <th className="border-0" style={{ width: '10%' }}>Begin</th>
                  <th className="border-0" style={{ width: '10%' }}>End</th>

                </tr>
              </thead>
              <tbody>
                {tasks.data.map?.(c => <TableRow key={`command-${c.id}`} {...c} />)}
              </tbody>
            </Table>
          </Card.Body>
        </Card>
      ) : 'loading...'}
    </>
  )
}