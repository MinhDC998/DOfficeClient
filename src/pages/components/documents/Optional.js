...
function Optional({ documentId, nameDocument, is_lock, author, format, type, updateInfo, reupdateDocument, reupdateDelete }) {
  ...
  const [nameDoc, setNameDoc] = useState(nameDocument)
  ...
  const handleReport = async () => {
    const result = await documentServices.reportToAdmin(documentId)
    if(result.code === '200') {
      alertSuccess("Đã gửi yêu cầu tới quản trị hệ thống!")
    }
  }
    ...
    <>
      <Dropdown as={ButtonGroup} className="my-3 m-lg-1 dropdown-style">
        ...
        <Dropdown.Menu>
          ...
          { is_lock === 1 && type === "public" && usrname !== lockInfo.username &&
            <Dropdown.Item
      ...
      >
        ...
        <Modal.Body>
          ...
          {judge().isWord == true && <iframe src={urlPreview} />}
